...
def test_model(model, test_datasets):
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(script_dir, "plots_input_temp_only")
    ...
    for idx, dataset in enumerate(test_datasets):
        ...
        for t in range(x.shape[0]):
            # 只使用 Input Temperature 作为输入
            current_input_temp = x[t, 2]  # Input Temperature

            # 输入只包含 Input Temperature（1个特征）
            input_t = torch.tensor([[current_input_temp]],
        ...
        dummy_actual[:, 0] = ft[1:len(pred_seq) + 1]  # Time
        ...
        dummy_pred[:, 0] = ft[1:len(pred_seq) + 1]  # Time
        ...
        plt.title(f"Prediction (Input Temp Only) - {file}")
        plt.legend()
