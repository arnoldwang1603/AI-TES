...
def test_model(model, test_datasets):
    ...
    for idx, dataset in enumerate(test_datasets):
        ...
        dummy_actual[:, 0] = ft[1:len(pred_seq) + 1]
        dummy_actual[:, 1] = ft_min[1:len(pred_seq) + 1]
        dummy_actual[:, 2] = ft_max[1:len(pred_seq) + 1]
        dummy_actual[:, 3] = ft_ave[1:len(pred_seq) + 1]
        dummy_actual[:, 4] = x[:, 2].cpu().numpy()

        dummy_pred = np.zeros((len(pred_seq), 5))
        dummy_pred[:, 0] = ft[1:len(pred_seq) + 1]
        dummy_pred[:, 1] = pred_seq[:, 1]
        dummy_pred[:, 2] = pred_seq[:, 0]
        dummy_pred[:, 3] = pred_seq[:, 2]
        dummy_pred[:, 4] = x[:, 2].cpu().numpy()
        ...
        prediction_df = pd.DataFrame({
            'Time (s)': inv_pred[:, 0],
            'T_outer (Predicted_GRU)': inv_pred[:, 1],
            'T_inner (Predicted_GRU)': inv_pred[:, 2],
            'T_avg (Predicted_GRU)': inv_pred[:, 3]
        })

        # 保存预测结果CSV
        csv_filename = os.path.splitext(file)[0] + "_predictions.csv"
        csv_path = os.path.join(output_dir, csv_filename)
        prediction_df.to_csv(csv_path, index=False, float_format='%.6f')
        ...
        mae_overall = (mae_inner + mae_outer + mae_avg) / 3

        # 记录MAE数据
        mae_records.append({
            'File': file,
            'MAE_T_inner': mae_inner,
            'MAE_T_outer': mae_outer,
            'MAE_T_avg': mae_avg,
    ...
    avg_mae = {
        'File': 'AVERAGE',
        'MAE_T_inner': mae_df['MAE_T_inner'].mean(),
        'MAE_T_outer': mae_df['MAE_T_outer'].mean(),
        'MAE_T_avg': mae_df['MAE_T_avg'].mean(),
        'MAE_overall': mae_df['MAE_overall'].mean()
    }
    mae_df = pd.concat([mae_df, pd.DataFrame([avg_mae])], ignore_index=True)

    # 保存CSV
    csv_path = os.path.join(output_dir, "mae_results.csv")
    mae_df.to_csv(csv_path, index=False, float_format='%.4f')

    print(f"\n{'=' * 60}")
    print(f"MAE Results Summary:")
    print(f"{'=' * 60}")
    print(f"Average MAE_T_inner: {avg_mae['MAE_T_inner']:.4f}°C")
    print(f"Average MAE_T_outer: {avg_mae['MAE_T_outer']:.4f}°C")
    print(f"Average MAE_T_avg:   {avg_mae['MAE_T_avg']:.4f}°C")
    print(f"Average MAE_overall: {avg_mae['MAE_overall']:.4f}°C")
    print(f"{'=' * 60}")
    print(f"\nMAE results saved to: {csv_path}")
